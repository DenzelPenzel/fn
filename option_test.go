...
func TestElimOption(t *testing.T) {
	result := ElimOption(Some(3), "none", func(_ int) string { return "some" })
	assertEqual(t, result, "some")

	result2 := ElimOption(None[int](), "none", func(_ int) string { return "some" })
	assertEqual(t, result2, "none")
}

func TestMapOption(t *testing.T) {
	mapped := MapOption(func(_ int) string { return "ok" }, Some(1))
	assertTrue(t, mapped.IsSome())
	assertEqual(t, mapped.UnsafeFromSome(), "ok")

	mapped2 := MapOption(func(_ int) string { return "ok" }, None[int]())
