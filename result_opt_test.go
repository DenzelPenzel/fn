...
func TestMapResultOpt(t *testing.T) {
	ro := OkOpt(5)
	mapped := MapResultOpt(func(i int) string { return "ok" }, ro)
	assertTrue(t, mapped.IsSome())

	ro2 := NoneOpt[int]()
	mapped2 := MapResultOpt(func(i int) string { return "ok" }, ro2)
	assertTrue(t, mapped2.IsNone())

	ro3 := ErrOpt[int](errors.New("e"))
	mapped3 := MapResultOpt(func(i int) string { return "ok" }, ro3)
	assertTrue(t, mapped3.IsErr())
}

func TestAndThenResultOpt(t *testing.T) {
	ro := OkOpt(5)
	result := AndThenResultOpt(ro, func(i int) ResultOpt[string] {
		return OkOpt("five")
	})
	assertTrue(t, result.IsSome())

	ro2 := NoneOpt[int]()
	result2 := AndThenResultOpt(ro2, func(i int) ResultOpt[string] {
		return OkOpt("x")
	})
	assertTrue(t, result2.IsNone())

	ro3 := ErrOpt[int](errors.New("e"))
	result3 := AndThenResultOpt(ro3, func(i int) ResultOpt[string] {
		return OkOpt("x")
	})
