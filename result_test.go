...
func TestResultFlatMap(t *testing.T) {
	...
	r2 := Err[int](errors.New("e")).FlatMap(func(i int) Result[int] {
		return Ok(0)
	})
	assertTrue(t, r2.IsErr())
}
...
func TestMapResultOk(t *testing.T) {
	r := MapResultOk(func(i int) string { return "ok" }, Ok(5))
	assertTrue(t, r.IsOk())
	assertEqual(t, r.UnwrapOr(""), "ok")

	r2 := MapResultOk(func(i int) string { return "ok" }, Err[int](errors.New("e")))
	assertTrue(t, r2.IsErr())
}
